...
import cn.finalteam.galleryfinal.utils.ImageCompress;
...
public abstract class PhotoBaseActivity extends Activity implements EasyPermissions.PermissionCallbacks{
    ...
    protected void takePhotoAction() {
        ...
        String imageName = "IMG" + DateUtils.format(new Date(), "yyyyMMddHHmmss") + ".jpg";
        File toFile = new File(takePhotoFolder, imageName);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if ( requestCode == GalleryFinal.TAKE_REQUEST_CODE ) {
            if (resultCode == RESULT_OK && mTakePhotoUri != null) {
                ...
                if (new File(path).exists()) {
                	...
                	ArrayList<PhotoInfo> mSelectPhotoList = new ArrayList<>();
                	...
                	startActivityForResult(intent, GalleryFinal.PREVIEW_REQUEST_CODE);
                    /*updateGallery(path);
                    takeResult(info);*/
                } else {
                    takePhotoFailure();
        ...
        }else if(requestCode == GalleryFinal.PREVIEW_REQUEST_CODE){
        	 if (resultCode == RESULT_OK && mTakePhotoUri != null){
        		 boolean isSource = false;
        		 String path = mTakePhotoUri.getPath();
        		 if(data != null){
        			 isSource = data.getBooleanExtra("isSource", true);
        		 }
                 if (new File(path).exists()) {
                	 PhotoInfo info = new PhotoInfo();
                	 info.setPhotoId(Utils.getRandom(10000, 99999));
                	 if(!isSource){
            			info.setPhotoPath(compressImage(path));
            		 }else{
            			 info.setPhotoPath(path);
            		 }
                     updateGallery(path);
	...
	private String compressImage(String path) {
		...
		options.srcFilePath = path;
		String destFilePath = path+"compress.jpg";
		options.destFilePath = destFilePath;
		compress.compressFromFilePath(this, options);
